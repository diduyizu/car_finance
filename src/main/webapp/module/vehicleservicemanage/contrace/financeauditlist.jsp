...
<form class="form-inline definewidth m20" action="${ctx}/vehicleservice/riskcontrol/audit" method="post">
    ...
    <select id="status" name="status">
        ...
        <option value="业务经理通过" <c:if test="${status == '业务经理通过'}">selected="selected"</c:if>>业务经理通过</option>
    </select>
    姓名：
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <c:forEach var="reservation" items="${reservation_list}" varStatus="status">
        <tr>
            ...
            <td>
                <c:if test="${reservation.status == '业务经理通过'}">
...
<script>
    $(function () {
        $('.pass').click(function(){
            var id = $(this).val();
            $.ajax({
                url:"${ctx}/vehicleservice/finance/doaudit",
                type: "post",
                data:{id:id,status:'财务通过'},
