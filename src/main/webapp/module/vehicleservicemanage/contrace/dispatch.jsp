        ...
        <table class="table table-bordered table-hover definewidth m10">
            ...
            <c:forEach var="vehicle" items="${vehicle_contrace_vehs_list}" varStatus="status">
                <tr>
                    ...
                    <td>${vehicle.model}</td>
                    <td>
                        <c:if test="${vehicle.dispatch_status == 1}">${vehicle.km}</c:if>
                        <c:if test="${vehicle.dispatch_status == 0}">
                            <input type="text" class="km" name="km" placeholder="必填" required="true" value="${vehicle.km}" />
                        </c:if>
                    </td>
                    <td>
                        <c:if test="${vehicle.dispatch_status == 1}">${vehicle.oil_percent}</c:if>
                        <c:if test="${vehicle.dispatch_status == 0}">
                            <input type="text" class="oil_percent" name="oil_percent" placeholder="必填" required="true" value="${vehicle.oil_percent}" />
                        </c:if>
                    </td>
...
<script>
    $(function () {
        ...
        $(".dispatch").click(function(){
            ...
            var km = $(this).parent('td').parent('tr').find("input[name='km']").val();
            var vehicle_contrace_id = $(this).next("input").val();
            var oil_percent = $(this).parent('td').parent('tr').find("input[name='oil_percent']").val();


            $.ajax({
                url:"${ctx}/vehicleservice/contrace/dodispatch",
                type: "post",
                data:{contrace_id:contrace_id,km:km,vehicle_contrace_id:vehicle_contrace_id,oil_percent:oil_percent},
